...
export class CheckoutComponent implements OnInit {
  ...
  form: FormGroup = this.formBuilder.group({
    tenKhachHang: [
      this.cacheService.get('customer').hoTen,
      [Validators.required, Validators.maxLength(50)],
    ],
    soDienThoai: [
      this.cacheService.get('customer').soDienThoai,
      [
        Validators.required,
    ...
    if (this.form.valid) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          // check confirm
          if (this.form.value.phuongThucThanhToan === '0') {
            // check phuongthucthanhtoan
            this.checkoutService.checkout(this.form.value).then(
              (c) => {
  ...
  callApiDistrict(province_id: string) {
    ...
    this.http
      .get(this.hostGHN + 'district?province_id=' + province_id, { headers })
  ...
  callApiWard(district_id: string) {
    ...
    this.http
      .get(this.hostGHN + 'ward?district_id=' + district_id, { headers })
      .subscribe((response: any) => {
        console.log(response);
