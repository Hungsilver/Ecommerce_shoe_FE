...
export class dialogProductDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.productDetailForm = this.fb.group({
      soLuong: [
        '',
        [Validators.required, Validators.pattern(/^[1-9]\d{0,2}$/)],
      ],
      giaBan: [
        '',
        [
          Validators.required,
          Validators.pattern(/^[1-9]\d{0,9}(\.\d{1,3})?$/),
          Validators.min(1000),
        ],
      ],
    ...
    if (
      ...
      this.data.productDetail.soLuong &&
      this.data.productDetail.giaBan &&
      this.data.productDetail.sanPham &&
      this.data.productDetail.mauSac &&
      this.data.productDetail.kichCo &&
      this.data.productDetail.chatLieuGiay &&
      this.data.productDetail.chatLieuDeGiay
    ) {
